...
from Aggregators.fltg import FLTG
...
aggr_mapper = {'cc': Clipping, 'cm': CM, 'krum': Krum, 'rfa': RFA, 'tm': TM,'avg':fedAVG,
               ...
               'gas':Gas,'fl_trust':FL_trust,'fltg':FLTG}
...
def get_aggr(args,test_set,net_ps,device):
    ...
    p = args.gas_p
    root_dataset = None
    if alg == 'fl_trust' or alg == 'fltg':
        l = test_set.__len__()

        if getattr(args, "root_bias_path", None):
            import json
            with open(args.root_bias_path, "r", encoding="utf-8") as f:
                root_dataset_inds = json.load(f)
        else:
...
#        root_dataset = DatasetSplit(test_set,root_dataset_inds)
    aggr_params = {'cc': [args.tau], 'ccs':[args.tau,args.buck_len,args.buck_rand,args.buck_avg]
        ,'cm': [None],'sign': [None],'krum': [num_client,b,n], 'rfa': [args.T,args.nu], 'tm': [b],'avg':[None],'bulyan':[num_client,b],
                   'gas':[num_client,b,n,p,secondry_alg],'fl_trust':[root_dataset,net_ps,args,device],'fltg':[root_dataset,net_ps,args,device]}
