...
def plot_results(data: Dict[float, Dict[float, Dict[str, float]]], aggregators: List[str], output: Path) -> None:
    ...
    for ax, bias in zip(axes, biases):
        ...
        for aggr in aggregators:
            ...
            ax.plot([r * 100 for r in ratios], accs, marker="o", label=aggr)
...
def main():
    parser = argparse.ArgumentParser(description="Plot CIFAR Fig.3 style results.")
    parser.add_argument("--run_dir", required=True, help="new_results_fig3_cifar/<timestamp> directory")
    parser.add_argument("--aggregators", default="fl_trust,fltg")
    parser.add_argument("--output", default="new_fig3_cifar.png")
